...
void Initialize() {
  const auto user_buffer = new BufferPoolManager(400, make_unique<DiskManager>("user.dat"));
  ...
  const auto station_buffer = new BufferPoolManager(400, make_unique<DiskManager>("station.dat"));
  const auto waitlist_buffer = new BufferPoolManager(200, make_unique<DiskManager>("waitlist.dat"));
  const auto orderlist_buffer = new BufferPoolManager(270, make_unique<DiskManager>("orderlist.dat"));
  const auto ticket_buffer = new BufferPoolManager(270, make_unique<DiskManager>("ticket.dat"));
