...
BPLUSTREE_TYPE::BPlusTree(unique_ptr<BufferPoolManager> buffer_pool_manager,
  ...
  : bpm_(std::move(buffer_pool_manager)),
    ...
    header_page_id_(0) {
  if (buffer_pool_manager->IsFirstVisit()) {
...
auto BPLUSTREE_TYPE::Insert(const KeyType &key, const ValueType &value) -> bool {
  ...
  std::pair<KeyType, ValueType> tmp[leaf_max_size_ + 1];
  for (int i = 0; i < pos; ++i) {
    tmp[i] = std::make_pair(leaf_page->KeyAt(i), leaf_page->ValueAt(i));
  }
  tmp[pos] = std::make_pair(key, value);
  for (int i = pos; i < cur_size; ++i) {
    tmp[i + 1] = std::make_pair(leaf_page->KeyAt(i), leaf_page->ValueAt(i));
  }
...
void BPLUSTREE_TYPE::InsertInternal(const KeyType &key, Context &ctx, int ch) {
  ...
  std::pair<KeyType, page_id_t> tmp[internal_max_size_ + 1];
  for (int i = 0; i < pos; ++i) {
    tmp[i] = std::make_pair(cur_page->KeyAt(i), cur_page->ValueAt(i));
  }
  tmp[pos] = std::make_pair(key, ch);
  for (int i = pos; i < cur_page->GetSize(); ++i) {
    tmp[i + 1] = std::make_pair(cur_page->KeyAt(i), cur_page->ValueAt(i));
  }
  int new_id;
  auto new_guard = bpm_->NewPageGuarded(&new_id);
...
void BPLUSTREE_TYPE::Remove(const KeyType &key) {
  ...
  auto pos = leaf_page->LowerBound(key, comparator_);
  if (pos >= cur_page->GetSize() || comparator_(leaf_page->KeyAt(pos), key) != 0) {
    return;
  }
  auto cur_size = leaf_page->GetSize();
